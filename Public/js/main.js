// file to add post to the main page
async function getPosts() {
  return await fetch("http://localhost:3000/posts")
    .then((response) => response.json()) //converting encrypted file into readable format f.ex. json
...
callMeForm.addEventListener("submit", function (e) {
  ...
  fetch("http://localhost:3000/callback-requests", {
    ...
    .then((resp) => resp.text())
    .then(() => alert("We will call you as sonn as possible"));
});
